...
public class ContentReferenceUtil extends ContentReference {
    ...
    public ContentReferenceUtil(ContentId referredContentId,
            ContentId referenceMetaDataId, ContentListRead contentList, PolopolyContext context) {
    ...
    public ContentReferenceUtil(ContentReference entry, ContentListRead contentList, PolopolyContext context) {
        this(entry.getReferredContentId(), entry.getReferenceMetaDataId(), contentList, context);
    }

    public <T> T getMetaData(Class<T> policyClass) throws NoReferenceMetaDataException, PolicyGetException {
        ContentId referenceMetaDataId = super.getReferenceMetaDataId();

        if (referenceMetaDataId == null) {
            throw new NoReferenceMetaDataException("No reference metadata available for " + this + ".");
        }
    ...
    public String toString() {
        return "reference to " + Util.util(super.getReferredContentId(), context) +
            (contentList instanceof ContentListUtil ? " in " + contentList.toString() : "");
    }
}
