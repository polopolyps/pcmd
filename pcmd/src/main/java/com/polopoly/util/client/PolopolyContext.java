...
public class PolopolyContext {
    ...
    public CMServer getCMServer() {
        if (cmServer == null) {
            throw new ServiceUnattachedException("CM server not available due to the way the context was constructed.");
        }
    ...
    public <T> T createPolicy(int major, String inputTemplateName, ContentId securityParent, Class<T> klass,
        ...
        try {
            ...
            throw new PolicyCreateException("The input template \"" + inputTemplateName + "\" could not be used: "
                                            + e.getMessage(), e);
        }
    ...
    public <T> T createPolicy(int major, ContentId inputTemplate, ContentId securityParent, Class<T> klass,
        ...
        try {
            ...
            throw new PolicyCreateException("Could not create content with template " + toString(inputTemplate) + ": "
                                            + e.getMessage(), e);
    ...
    public <T> T getUser(UserId userId, Class<T> userClass) throws UserNotFoundException {
        try {
            ...
            String message =
                "Fetching current user with principal ID " + userId.getPrincipalIdString() + ": " + e.getMessage();
    ...
    /**
     * Retrieves the solrSearch client matching the indexName set in the context
     * or in the app.
     * 
     * @param indexName
