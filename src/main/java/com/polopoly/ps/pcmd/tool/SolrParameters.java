...
public class SolrParameters implements Parameters {
    ...
    public void parseParameters(Arguments args, PolopolyContext context) throws ArgumentException {
        ...
        if (args.getArgumentCount() == 0) {
            throw new ArgumentException(
                "There were no arguments (possibly only options). The search query should be specified as arguments.");
        }
    ...
    public void getHelp(ParameterHelp help) {
        ...
        help.addOption(INSPECT_OPTION, new BooleanParser(),
                       "Show stored fields in Solr document according to Solr search result's content ID; defaults to false).");
        help.addOption(FILTER_QUERY_OPTION,
                       null,
                       "The filter query that can be used to restrict the super set of documents that can be returned, without influencing score");
    }
