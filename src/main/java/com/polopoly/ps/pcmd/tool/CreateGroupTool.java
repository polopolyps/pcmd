...
public class CreateGroupTool implements Tool<CreateGroupParameters> {
	...
	public void execute(PolopolyContext context,
			CreateGroupParameters parameters) {
		...
		for (String groupNameToCreate : parameters.getGroupNamesToCreate()) {
			...
			try {
				groupIds = context.getUserServer().findGroupsByName(
						groupNameToCreate);
			} catch (RemoteException e) {
				throw new FatalToolException("While fetching groups: "
						+ e.getMessage(), e);
			}
			...
			if (groupIds != null && groupIds.length > 0) {
				if (groupIds.length == 1) {
					try {
						...
						System.err
								.println(groupNameToCreate
										+ " already exists. Using it to add memebers and owners.");
					} catch (Exception e) {
						System.err.println("While looking up group name: "
								+ groupNameToCreate + ": " + e.toString());

						continue;
					}
				} else {
					System.err.println("Group name: " + groupNameToCreate
							+ " is ambiguous. " + groupIds.length
							+ " groups match.");
				...
				try {
					group = context.getUserServer().createGroup(
							currentCaller.getUserId());

					group.setName(groupNameToCreate, currentCaller);

					System.out.println(AbstractPrincipalIdField.get(
							group.getGroupId(), context));
				} catch (Exception e) {
					System.err.println("While creating group "
							+ groupNameToCreate + ": " + e.toString());
			...
			for (PrincipalId member : parameters.getMembers()) {
				try {
					...
					System.err.println("While adding member "
							+ AbstractPrincipalIdField.get(member, context)
			...
			for (PrincipalId owner : parameters.getOwners()) {
				try {
					...
					System.err.println("While adding owner "
							+ AbstractPrincipalIdField.get(owner, context)
							+ " to " + groupNameToCreate + ": " + e.toString());
				}
			}
		}
